...
//实现接口的结构体
type Point struct {
	//x, Y int//私有的额字段不允许，如果你需要对私有字段进行编码解码的话，
	// 那么就要实现GobEncoder和GobDecoder接口，在func (t Time) GobEncode(){}有实现这个接口，你可以去看看
	X, Y int
}
...
func Example_interface() {
	...
	gob.Register(Point{})//指明要编码哪个结构体,必须在gob实例化编码器或者解码器之前调用

	//事实上RegisterName是Register的底层实现，Register的参数是默认的类型名字，但是RegisterName的参数是完全的包名+类型名字，仅此而已，
	//在绝大多数情况下都应该使用Register，因为这更加方便！其他的东西跟上面的Register的用法完全一致，因此不在这累叙了！
	//gob.RegisterName()
